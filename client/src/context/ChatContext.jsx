...
export const ChatProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const onNewMessage = ({ message, conversation }) => {
      ...
      setMessages((prev) => {
        if (activeChat?.type === 'dm' && activeChat.data._id === conversation._id) {
  ...
  const openDM = useCallback(async (targetUser) => {
    const { data } = await api.post('/conversations', { receiverId: targetUser._id });
    setActiveChat({ type: 'dm', data: data.conversation });
    setLoadingMessages(true);
    try {
      const msgRes = await api.get(`/messages/conversation/${data.conversation._id}`);
  ...
  return (
    <ChatContext.Provider value={{
      conversations, groups, activeChat, messages, onlineUsers,
      typingUsers, loadingMessages,
      loadConversations, openDM, openGroup, sendMessage,
